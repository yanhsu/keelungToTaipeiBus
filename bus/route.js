...
class Route {
  ...
  async getAllEstimateTimeByRouteId(routeId, direction) {
	try {
		return await axios({
			url: `${baseUrl}/Bus/EstimatedTimeOfArrival/Streaming/InterCity?$filter=contains(RouteName%2FZh_tw%20%20%2C%20'${routeId}')%20and%20Direction%20eq%20'${direction}'&$orderby=StopSequence&$format=JSON`,
			method: 'get',
			headers: await getAuthorizationHeader(),
		});
	} catch (error) {
