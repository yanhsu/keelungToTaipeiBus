...
class Route {
  ...
  async getAllEstimateTimeByRouteId(routeId, direction) {
	...
	async getAllEstimateTimeByRouteIdWithoutDirection(routeId) {
		try {
			return await axios({
				url: `https://ptx.transportdata.tw/MOTC/v2/Bus/EstimatedTimeOfArrival/Streaming/InterCity?$filter=contains(RouteName%2FZh_tw%20%20%2C%20'${routeId}')&$orderby=StopSequence&$format=JSON`,
				method: 'get',
				headers: getAuthorizationHeader(),
			});
		} catch (error) {
